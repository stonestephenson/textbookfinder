...
**How the units number gets seeded.** You always type or adjust it, but the field starts from an
estimate so most students don't have to. When a screenshot is read, the tool sums the units of
your registered courses — *all* of them, including courses with no materials — using a
course→units table built from the term's published Schedule of Classes (`assets/course-units.js`,
generated by `scripts/build-course-units.mjs`; see README). Course codes join the table after
normalizing (`CS 454 001` → `CS 454`); a variable-unit course (research, internship, independent
study) uses the rounded median of the values it is offered at this term; the rare course the
table doesn't carry contributes `config.assumedUnitsPerCourse` (3). The estimate replaces the
flat 15 baseline **only** when the read looks complete (the capture wasn't flagged cut off), the
table's term matches the deployed term, and you haven't already set units yourself. It is a
**better default, never an assertion**: the schedule knows a course's offered units, not which
section or unit count you personally chose, so the number stays fully editable on the confirm
screen and nothing is computed until you confirm it. The paste and manual paths keep the 15
baseline (no full course list to sum).
