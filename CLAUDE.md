...
- **Done-gate:** `bash .claude/verify.sh` (syntax-checks all JS, runs `node --test`). Green
  before any commit.
- **Test-first** for the six pure modules — `assets/verdict.js`, `assets/parse-text.js`,
  `assets/estimate-units.js`, `api/_pick-offer.js`, `api/_resolve-match.js`,
  `scripts/parse-schedule.js` — each fully covered in `tests/`. Never weaken a test to make it
  pass.
...
- **The units estimate seeds, never asserts.** `assets/course-units.js` is GENERATED by
  `scripts/build-course-units.mjs` from the term's Schedule of Classes (in `data/`) — do not
  hand-edit it; regenerate **per semester** (README). It only pre-fills the units field on the
  screenshot path, gated on a complete capture + matching term + untouched units, and stays
  editable. Paste and manual entry keep the flat 15 baseline. See ARCHITECTURE "The units estimate".
