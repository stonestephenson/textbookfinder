...
- **Bookstore email** — email thread between the author and SSU bookstore staff, July 2026.
  Retained by the author; quoted verbatim in `SEAWOLF_BUNDLE_CONTEXT.md` §3.
- **Portal screenshot** — screenshots of the author's own bookstore portal, Fall 2026 registration,
  July 2026. Retained by the author.
...
| 5 | Opting out **closes at the add/drop deadline** | SSU bookstore program terms (opt-out window aligned to add/drop); no specific date is shown unless `config.optOutDeadline` carries a sourced one |
...
| 13 | The student price **rose from $18.50/unit (2023) to $21.00 now** | 2023: documented student-facing launch rate of $18.50/unit (`SEAWOLF_BUNDLE_CONTEXT.md` §2, verified); current: bookstore email (claim #1) |
| 14 | The author's Fall 2026 cart showed **one included item across 10 registered units** | Portal screenshot |
| 15 | That item was a paperback selling online for **about $11 new (July 2026)** | Author's price capture, July 2026 (~$11.08 new online); `SEAWOLF_BUNDLE_CONTEXT.md` §3 |
...
| 19 | **Professors can add materials after the term starts** (late adoptions are permitted by the contract) | Contract, Amendment No. 1 §22(i)(2) (late adoptions permitted; B&N "cannot guarantee delivery" but will use reasonable efforts) |
...
| 24 | This site **never asks for SSU credentials**; pasted text is parsed **in the browser**; screenshots are **processed in a single request and not stored**; **no analytics, no tracking, no accounts** | Code: `assets/parse-text.js` runs client-side; `api/parse.js` writes nothing and logs no image data; no analytics scripts exist in this repository |
...
## Claims deliberately NOT made on the site

These are open questions, not findings. They must not appear in the UI in any form
(`SEAWOLF_BUNDLE_CONTEXT.md` §5 is the authority):

- That SSU or SSE **marks up** the per-unit price.
- That the contract **bars SSU from publishing cost comparisons** (§22(iv) says "external
  communications"; the site does not interpret it).
- **Which rate** produced the portal's $67.15 savings figure (unknown; the site quotes the figure
  and states adjacent facts only).
- That anyone **intended** any of this. The site describes structure, not motive.
- Any statement about **what a course requires**. The site only ever describes what *the user's own
  cart shows*.
