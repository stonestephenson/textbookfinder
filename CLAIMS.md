...
| 36 | Found prices are **"real offers at the moment of lookup"**, each shown with its store, condition or rental length, **shipping included**, and a link to the offer | Code: `api/price.js` queries BooksRun's public buy-price API live per request and caches nothing; `api/pick-offer.js` selects the cheapest purchasable offer (used, new, or a term-length rental of 110+ days), always price plus shipping; the UI labels each with its provenance and links to the product page. Offers can change between lookup and purchase — the UI says so ("offers move, so buy soon after you decide") |
| 37 | For an access code, a price labeled **"listed in your own capture"** is the price printed in the user's uploaded screenshot/PDF | Code: `api/parse.js` extracts `listedPrice` only when a price is legibly printed next to an item (the prompt forbids estimating); `applyCapturePrices()` in `assets/app.js` applies it **only to access-code items**, never to books, and only when the user hasn't set a price; the label states the provenance and the value is editable |
